...
const periodBadgeClass: Record<string, string> = {
  yearly: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/50 dark:text-emerald-300',
...
<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative w-full max-w-lg max-h-[90vh] overflow-y-auto bg-white dark:bg-[#382933] rounded-3xl shadow-2xl p-8 border border-stone-100 dark:border-[rgba(255,255,255,0.065)] animate-in fade-in zoom-in duration-200">
      <!-- Header -->
      <div class="flex justify-between items-center mb-6">
        <div class="flex items-center gap-2">
          <Sparkles :size="20" class="text-emerald-500" />
          <h2 class="text-2xl font-bold text-[#382933] dark:text-[#e8ede6]">{{ t('architect.title') }}</h2>
        </div>
      ...
      <div v-if="state === 'input'" class="space-y-5">
        ...
        <div class="flex flex-wrap gap-2">
          <button
            ...
            class="text-xs px-3 py-1.5 rounded-lg bg-stone-100 dark:bg-[#42313c] text-[#a4b494] hover:bg-stone-200 dark:hover:bg-[#3b5249] transition-colors cursor-pointer"
          >
            {{ example }}
          </button>
        </div>
        ...
        <button
          ...
          class="w-full py-4 bg-[#519872] text-white rounded-2xl font-bold hover:bg-[#3d7a5a] transition-colors shadow-xl shadow-[#519872]/20 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2"
        >
          <Sparkles :size="16" />
      ...
      <div v-else-if="state === 'loading'" class="flex flex-col items-center justify-center py-12 gap-4">
        ...
        <p class="text-sm font-medium text-[#a4b494]">{{ t('architect.generating') }}</p>
        <div class="w-full space-y-3 mt-4">
          <div v-for="i in 4" :key="i" class="h-10 bg-stone-100 dark:bg-[#42313c] rounded-xl animate-pulse" :style="{ animationDelay: `${i * 150}ms` }"></div>
        </div>
      </div>
      ...
      <div v-else-if="state === 'preview'" class="space-y-5">
        ...
        <template v-for="(group, key) in groupedGoals" :key="key">
          <div v-if="group.length > 0">
            <h3 class="text-xs font-bold uppercase tracking-widest text-[#a4b494] dark:text-[#637562] mb-2">
              {{ t(`architect.${key}`) }}
            </h3>
            <div class="space-y-2">
              <div
                ...
                :class="goal.selected
                  ? 'bg-white dark:bg-[#42313c]/50 border-stone-200 dark:border-[rgba(255,255,255,0.065)]'
                  : 'bg-stone-50 dark:bg-[#382933] border-stone-100 dark:border-[rgba(255,255,255,0.065)] opacity-50'"
              >
                <button
                  ...
                  :class="goal.selected
                    ? 'bg-[#519872] border-[#519872] text-white'
                ...
                <div class="flex-1 min-w-0">
                  <input
                    v-model="goal.title"
                    class="w-full text-sm font-semibold text-[#382933] dark:text-[#e8ede6] bg-transparent outline-none focus:ring-1 focus:ring-primary/30 rounded px-1 -ml-1"
                  />
                  <p class="text-xs text-[#a4b494] dark:text-[#637562] mt-0.5 px-1">{{ goal.description }}</p>
                </div>
        ...
        <div class="flex gap-3 pt-2">
          <button
            ...
            class="flex-1 py-3 rounded-2xl font-bold text-sm border border-stone-200 dark:border-[rgba(255,255,255,0.065)] text-[#637562] dark:text-[#a4b494] hover:bg-stone-50 dark:hover:bg-[#42313c] transition-colors flex items-center justify-center gap-2 cursor-pointer"
          >
            <RefreshCw :size="14" />
          ...
          <button
            ...
            class="flex-1 py-3 bg-[#519872] text-white rounded-2xl font-bold text-sm hover:bg-[#3d7a5a] transition-colors shadow-lg shadow-[#519872]/20 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2 cursor-pointer"
          >
            <Check :size="14" />
        ...
        <button
          @click="$emit('close')"
          class="w-full py-2 text-sm text-[#a4b494] hover:text-[#637562] dark:hover:text-[#a4b494] transition-colors cursor-pointer"
        >
          {{ t('architect.cancel') }}
        </button>
      </div>
