...
const periodBadgeClass: Record<string, string> = {
  yearly: 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/50 dark:text-indigo-300',
...
<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative w-full max-w-lg max-h-[90vh] overflow-y-auto bg-white dark:bg-slate-800 rounded-3xl shadow-2xl p-8 border border-slate-100 dark:border-slate-700 animate-in fade-in zoom-in duration-200">
      <!-- Header -->
      <div class="flex justify-between items-center mb-6">
        <div class="flex items-center gap-2">
          <Sparkles :size="20" class="text-amber-500" />
          <h2 class="text-2xl font-bold text-slate-800 dark:text-slate-200">{{ t('architect.title') }}</h2>
        </div>
      ...
      <div v-if="state === 'input'" class="space-y-5">
        ...
        <div class="flex flex-wrap gap-2">
          <button
            ...
            class="text-xs px-3 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors cursor-pointer"
          >
            {{ example }}
          </button>
        </div>
        ...
        <button
          ...
          class="w-full py-4 bg-emerald-600 text-white rounded-2xl font-bold hover:bg-emerald-700 transition-colors shadow-xl shadow-emerald-200 dark:shadow-emerald-900/20 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2"
        >
          <Sparkles :size="16" />
      ...
      <div v-else-if="state === 'loading'" class="flex flex-col items-center justify-center py-12 gap-4">
        ...
        <p class="text-sm font-medium text-slate-500 dark:text-slate-400">{{ t('architect.generating') }}</p>
        <div class="w-full space-y-3 mt-4">
          <div v-for="i in 4" :key="i" class="h-10 bg-slate-100 dark:bg-slate-700 rounded-xl animate-pulse" :style="{ animationDelay: `${i * 150}ms` }"></div>
        </div>
      </div>
      ...
      <div v-else-if="state === 'preview'" class="space-y-5">
        ...
        <template v-for="(group, key) in groupedGoals" :key="key">
          <div v-if="group.length > 0">
            <h3 class="text-xs font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500 mb-2">
              {{ t(`architect.${key}`) }}
            </h3>
            <div class="space-y-2">
              <div
              ...
              >
                ...
                <div class="flex-1 min-w-0">
                  <input
                    v-model="goal.title"
                    class="w-full text-sm font-semibold text-slate-800 dark:text-slate-200 bg-transparent outline-none focus:ring-1 focus:ring-primary/30 rounded px-1 -ml-1"
                  />
                  <p class="text-xs text-slate-400 dark:text-slate-500 mt-0.5 px-1">{{ goal.description }}</p>
                </div>
        ...
        <div class="flex gap-3 pt-2">
          <button
            ...
            class="flex-1 py-3 rounded-2xl font-bold text-sm border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors flex items-center justify-center gap-2 cursor-pointer"
          >
            <RefreshCw :size="14" />
          ...
          <button
            ...
            class="flex-1 py-3 bg-emerald-600 text-white rounded-2xl font-bold text-sm hover:bg-emerald-700 transition-colors shadow-lg shadow-emerald-200 dark:shadow-emerald-900/20 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2 cursor-pointer"
          >
            <Check :size="14" />
        ...
        <button
          @click="$emit('close')"
          class="w-full py-2 text-sm text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors cursor-pointer"
        >
          {{ t('architect.cancel') }}
        </button>
      </div>
